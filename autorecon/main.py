...
async def scan_target(target):
  ...
  if not config['only_scans_dir']:
    ...
    # creation of header on report
    with open(os.path.join(reportdir, 'notes.txt'), 'a') as file:
        # title
        try:
            file.writelines(target.autorecon.args.engagement + '\n====================================\n\n')
        except e:
            print("add the --engagement flag")
    ...
    for service in services:
      ...
      if not config['only_scans_dir']:
        with open(os.path.join(target.reportdir, 'notes.txt'), 'a') as file:
                    file.writelines('#### ' + service.name + ': ' + service.protocol + '/' + str(service.port) + '\n- version: \n- notes: \n\n')
...
async def run():
  ...
  parser.add_argument('--version', action='store_true', help='Prints the AutoRecon version and exits.')
    # ra0x1duk3: added argument for storing the title of the engagement
  parser.add_argument('--engagement', action='store', help='Specify the engagement name for use in report title, if used will overwrite -o parameter')
